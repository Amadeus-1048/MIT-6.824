...
func (kv *KVServer) Command(request *CommandRequest, response *CommandResponse) {
	...
	kv.mu.RUnlock()                                     // 释放锁以提高吞吐量，确保Raft层可以继续提交日志。
	index, _, isLeader := kv.rf.Start(Command{request}) // 提交请求到Raft日志中
	if !isLeader {                                      // 如果当前节点不是领导者，赋值错误并返回
...
// 判断一个客户端请求是否是重复请求。
// 每个新的RPC请求都意味着客户端已经收到了之前的所有RPC请求的回复。因此，如果当前请求的ID小于等于记录的最大命令ID，则认为是重复请求。
...
// 获取用于通知客户端的通道。该通道在Raft日志条目被应用到状态机之后，通知等待结果的客户端
//
// index：Raft日志条目的索引，用于标识该日志条目
// chan：用于通知某索引对应日志条目应用结果的通道
func (kv *KVServer) getNotifyChan(index int) chan *CommandResponse { // 每个Raft日志条目都有一个对应的通道
...
// 当Raft日志条目被应用并且客户端已经被通知后，相关的NotifyChannel就不再需要了。为了避免内存泄漏和无用的资源占用，需要删除这些过时的通道
func (kv *KVServer) removeOutdatedNotifyChan(index int) {
	delete(kv.notifyChans, index)
}

// 将Raft日志条目应用到状态机，并生成相应的响应。
